...
`deploy` (передеплой по ssh). Условие
`if: always() && !failure() && !cancelled()` у `deploy` читается как
«запускайся, если ничего не упало»: без него деплой пропускался бы всегда, когда
одна из сборок skipped, — а так почти каждый раз, обычно меняется что-то одно.
...
Откатиться: **Actions → deploy → Run workflow** → вписать git-SHA нужного релиза
в поле `api_tag` или `web_tag` → Run. Джобы сборки при ручном запуске
...
остаются в `.env.prod` на сервере. Образ публичный.
...
- **Кеш слоёв требует отдельного билдера.** `cache-from/cache-to: type=gha` без
  ...
  драйвер `docker` умеет только собирать локально. Нужен драйвер
  `docker-container`, его и ставит этот шаг.
