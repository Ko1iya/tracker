...
- [ ] **Фронтенд: Бейдж «Категория уточняется…» (pending-флоу)**
  - [ ] В ленте у транзакций без категории, но с активным дедлайном
        (`autoConfirmAt !== null`), показать бейдж «Категория уточняется…».
        Помимо бейджа — короткий обратный отсчёт (например, «осталось 4 мин»)
        до авто-подтверждения крона.
  - [ ] При клике по бейджу раскрыть быстрый выбор: чипсы с вариантами из
        `suggestedCategories` (один тап = принять предложение) + поле «Своя
        категория» (свободный ввод).
  - [ ] Сабмит — `PATCH /transactions/:id/category` (новая функция
  ...
  - [ ] Учесть авто-подтверждение от крона: если пока пользователь думал, крон
        уже привязал категорию, после рефетча ленты бейдж естественно исчезает
        (флаг считается через `autoConfirmAt`, и он уже `null`).
