...
- [ ] **(Бэкенд) Поддержка категорий — нужно фронтенду**
  - [ ] Эндпоинт `GET /categories` (список категорий пользователя).
  - [ ] Принять `categoryId` в `CreateTransactionDto` (опциональное поле, связь
