...
└── src/
    ...
    │   ├── accounts/
    │   │   ├── api.ts         # getAccounts(), createAccount(), setDefaultAccount(), deleteAccount(); тип Account (+isDefault, transactionCount)
    ...
    │   │   └── hooks.ts       # useCategoriesQuery (чтение), useCreateCategoryMutation (optimistic), useDeleteCategoryMutation
    │   ├── transactions/
    ...
    │   │   ├── hooks.ts       # useTransactionsQuery, useCreateTransactionMutation, useCreateTransactionFromVoiceMutation, useSetTransactionCategoryMutation (все мутации инвалидируют кеш ленты)
    ...
    │   ├── BottomNav.tsx      # Нижняя панель мобильной версии: слева Настройки, справа три способа добавить расход (вручную · голос · фото)
...
- **`api.ts`** — функции запросов к budget-api (named exports). `getAccounts()` — `GET /accounts` (требует JWT), список счетов пользователя, отсортированный по `title`, каждый с `isDefault` и счётчиком `transactionCount`. `createAccount(title)` — `POST /accounts` (требует JWT): первый счёт пользователя бэкенд делает дефолтным, при дубликате (`userId+title`) отвечает 409. `setDefaultAccount(id)` — `PATCH /accounts/:id/default` (требует JWT): делает счёт дефолтным, снимая флаг с прежнего. `deleteAccount(id)` — `DELETE /accounts/:id` (требует JWT): удаляет любой счёт, включая непустой и дефолтный; транзакциям бэкенд обнуляет `accountId`, дефолт перевешивается на следующий по алфавиту. Тип `Account` (`id`, `title`, `isDefault`, `userId`, `transactionCount`).
- **`keys.ts`** — `accountKeys` (named export). Реестр query-ключей TanStack Query: `all: ["accounts"]` — под ним читается список и по нему же идёт инвалидация после мутаций.
- **`hooks.ts`** — `useAccountsQuery`, `useCreateAccountMutation`, `useSetDefaultAccountMutation`, `useDeleteAccountMutation` (named exports). `useAccountsQuery` — чтение списка через `useQuery` под ключом `accountKeys.all`. `useCreateAccountMutation` — мутация создания с оптимистичным добавлением в кеш (временная запись с отрицательным id, первый счёт помечается дефолтным локально; при ошибке откат, в `onSettled` инвалидация). `useSetDefaultAccountMutation` — мутация смены дефолта, флаг переставляется в кеше сразу, при ошибке откат. `useDeleteAccountMutation` — мутация удаления; в `onSettled` инвалидирует и `accountKeys.all`, и `transactionKeys.all` (у транзакций удалённого счёта меняется `accountId`). Оптимистичное скрытие на время undo-окна делает сам экран `AccountsPage`.
...
- **`hooks.ts`** — `useCategoriesQuery`, `useCreateCategoryMutation`, `useDeleteCategoryMutation` (named exports). `useCategoriesQuery` — чтение списка через `useQuery` под ключом `categoryKeys.all`. `useCreateCategoryMutation` — мутация создания с оптимистичным добавлением в кеш (временная запись с отрицательным id, при ошибке откат, в `onSettled` инвалидация). `useDeleteCategoryMutation` — мутация удаления; в `onSettled` инвалидирует `categoryKeys.all` (при успехе подтянет актуальный список, при 409 вернёт строку). Оптимистичное скрытие на время undo-окна делает сам экран `CategoriesPage`.

#### `src/features/transactions/`

- **`api.ts`** — функции запросов к budget-api (named exports). `getTransactions()` — `GET /transactions` (требует JWT), возвращает массив транзакций. `getTransaction(id)` — `GET /transactions/:id` (требует JWT), одна транзакция; 404 при отсутствии/чужой. `createTransaction(input)` — `POST /transactions` (требует JWT), создаёт транзакцию; `userId` бэкенд берёт из JWT, `currency`/`date` проставляет дефолтами. `updateTransaction(id, input)` — `PATCH /transactions/:id` (требует JWT), правит транзакцию (переданное подмножество полей). `deleteTransaction(id)` — `DELETE /transactions/:id` (требует JWT), возвращает `{ id }`. `createTransactionFromVoice(blob, filename)` — `POST /transactions/voice` (multipart, поле `audio`, требует JWT): отправляет записанное аудио, бэкенд транскрибирует и парсит LLM, возвращает уже созданную транзакцию. `setTransactionCategory(id, categoryName)` — `PATCH /transactions/:id/category` (требует JWT): подтверждает категорию для pending-транзакции (бэк найдёт-или-создаст категорию по имени, гасит `autoConfirmAt`). Типы `Transaction` (поле `amount` — строка, т.к. Prisma `Decimal`; `accountId: number | null` — счёт траты; плюс `suggestedCategories: string[]` и `autoConfirmAt: string | null` для pending-категории; опц. `categoryPending` — приходит только в ответах voice/PATCH), `TransactionType` (`INCOME` | `EXPENSE`), `CreateTransactionInput` (`amount`, `type`, опц. `description`, опц. `categoryId` — бэкенд проверит владение и привяжет категорию, опц. `accountId` — не передан, бэкенд подставит дефолтный счёт) и `UpdateTransactionInput` (= `Partial<CreateTransactionInput>`).
...
- **`hooks.ts`** — `useTransactionsQuery`, `useTransactionQuery`, `useCreateTransactionMutation`, `useUpdateTransactionMutation`, `useDeleteTransactionMutation`, `useCreateTransactionFromVoiceMutation`, `useSetTransactionCategoryMutation` (named exports). `useTransactionsQuery` — чтение списка под ключом `transactionKeys.all`. `useTransactionQuery(id)` — чтение одной транзакции под `transactionKeys.detail(id)` (для страницы детали при прямом переходе/обновлении). `useCreateTransactionMutation` — мутация создания. `useUpdateTransactionMutation` — мутация правки (на вход `{ id, input }`). `useDeleteTransactionMutation` — мутация удаления (на вход `id`). `useCreateTransactionFromVoiceMutation` — мутация голосового ввода (`{ audio, filename }`). `useSetTransactionCategoryMutation` — мутация подтверждения категории (`{ id, categoryName }`). Все мутации после успеха инвалидируют `transactionKeys.all` (благодаря общему префиксу освежается и лента, и детали); удаление и подтверждение категории дополнительно инвалидируют `categoryKeys.all` — у категорий меняется счётчик `transactionCount`.
- **`schema.ts`** — `createTransactionSchema` (zod) и тип `CreateTransactionFormValues` (named exports). Клиентская валидация формы создания; поля `amount`/`description`/`type` синхронны с `CreateTransactionDto` бэкенда (`amount` приводится из строки `<input>` через `z.coerce`). Категория задаётся одним из двух UI-полей: `categoryId` (id категории строкой — выбор радиокнопкой) либо `categoryInput` (название существующей/новой — ручной ввод); `superRefine` требует, чтобы было заполнено хотя бы одно. Готовый числовой `categoryId` собирает компонент. Поле `accountId` — id счёта строкой (значение Radix Select); пустая строка означает, что счетов у пользователя нет и поле не рендерится.
...
- **`AddTransactionDialog.tsx`** — `AddTransactionDialog` (default export, опц. пропы `trigger?: ReactNode`, `transaction?: Transaction`). Модальное окно добавления/правки транзакции: само хранит open-состояние. `trigger` заменяет элемент-триггер (например, круглая кнопка `BottomNav`); по умолчанию — кнопка «Добавить». Если передана `transaction` — режим правки: при открытии форма заполняется её значениями, заголовок «Редактировать транзакцию», сохранение шлёт `PATCH` через `useUpdateTransactionMutation` (иначе `POST` через `useCreateTransactionMutation`). Форма на `react-hook-form` + `zodResolver` (`createTransactionSchema`); поля Сумма, Описание, Тип (Расход/Доход), Категория (через `CategoryPicker`) и Счёт (`Select`, рендерится только если у пользователя есть хотя бы один счёт — список из `useAccountsQuery`; при создании предвыбран дефолтный, при правке — счёт транзакции либо дефолтный, если её счёт удалён; пустого варианта нет). Категории грузит через `useCategoriesQuery`, частоты — через `useTransactionsQuery` + `topCategoryIds` (мемоизированные топ-частых для радиокнопок, fallback — первые из справочника; в режиме правки текущая категория траты гарантированно добавлена в чипсы). В создании самая частая предвыбрана. Перед сохранением определяет `categoryId` (ручной ввод приоритетнее радиокнопки): по нормализованному имени ищет существующую, иначе создаёт через `useCreateCategoryMutation`.
...
- **`Layout.tsx`** — `Layout` (default export). Визуальный каркас авторизованных страниц: шапка с брендом и навигационными `NavLink` (Главная, Настройки), видимыми только на десктопе (`hidden sm:inline`). `<Outlet />` под вложенные страницы, снизу — `BottomNav` (мобильная навигация + действия). Выход вынесен на страницу «Настройки». Стили — Tailwind. Проверку доступа делает `ProtectedRoute` выше по дереву.
- **`BottomNav.tsx`** — `BottomNav` (default export). Фиксированная у нижнего края панель мобильной версии (`sm:hidden`). Слева — круглая кнопка `NavLink` «Настройки» (`/settings`; активный маршрут подсвечивается акцентом). Справа — ряд из трёх круглых кнопок единого стиля для добавления расхода: «Вручную» (`trigger` для `AddTransactionDialog`), «Голос» (`VoiceRecorderButton` в режиме `fab`) и «Фото» (распознавание чека — задел, кнопка `disabled`). Учитывает `safe-area-inset-bottom`.
...
| `/settings` | `SettingsPage`     | Настройки: выбор темы, ссылки на категории и счета + выход (внутри Layout, требует авторизации) | `src/pages/SettingsPage.tsx` | частично |
